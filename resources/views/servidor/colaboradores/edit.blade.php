                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="text" id="matricula" name="matricula"
                            value="{{ old('matricula', $servidor->matricula) }}"
                        ...
                        <select id="id_lotacao" name="id_lotacao"
                            ...
                            @foreach (\App\Models\Lotacao::all() as $lotacao)
                                <option value="{{ $lotacao->id }}"
                                    {{ old('id_lotacao', $servidor->id_lotacao) == $lotacao->id ? 'selected' : '' }}>
                        ...
                        <select id="id_vinculo" name="id_vinculo"
                            ...
                            @foreach (\App\Models\Vinculo::all() as $vinculo)
                                <option value="{{ $vinculo->id }}"
                                    {{ old('id_vinculo', $servidor->id_vinculo) == $vinculo->id ? 'selected' : '' }}>
                                    {{ $vinculo->nomeVinculo }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select id="tipoSanguineo" name="tipoSanguineo"
                            ...
                            <option value="" disabled>Selecione...</option>
                            <option value="A+"
                                {{ old('tipoSanguineo', $servidor->tipoSanguineo) == 'A+' ? 'selected' : '' }}>A+</option>
                            <option value="A-"
                                {{ old('tipoSanguineo', $servidor->tipoSanguineo) == 'A-' ? 'selected' : '' }}>A-</option>
                            <option value="B+"
                                {{ old('tipoSanguineo', $servidor->tipoSanguineo) == 'B+' ? 'selected' : '' }}>B+</option>
                            <option value="B-"
                                {{ old('tipoSanguineo', $servidor->tipoSanguineo) == 'B-' ? 'selected' : '' }}>B-</option>
                            <option value="AB+"
                            ...
                            <option value="O+"
                                {{ old('tipoSanguineo', $servidor->tipoSanguineo) == 'O+' ? 'selected' : '' }}>O+</option>
                            <option value="O-"
                                {{ old('tipoSanguineo', $servidor->tipoSanguineo) == 'O-' ? 'selected' : '' }}>O-</option>
                        </select>
                        @error('tipoSanguineo')
                    ...
                    <div>
                        ...
                        <select id="racaCor" name="racaCor"
                            ...
                            <option value="" disabled>Selecione...</option>
                            <option value="Branca" {{ old('racaCor', $servidor->racaCor) == 'Branca' ? 'selected' : '' }}>
                                Branca</option>
                            <option value="Preta" {{ old('racaCor', $servidor->racaCor) == 'Preta' ? 'selected' : '' }}>
                                Preta</option>
                            <option value="Parda" {{ old('racaCor', $servidor->racaCor) == 'Parda' ? 'selected' : '' }}>
                                Parda</option>
                            <option value="Amarela"
                                {{ old('racaCor', $servidor->racaCor) == 'Amarela' ? 'selected' : '' }}>Amarela</option>
                        </select>
                        @error('racaCor')
                            <span class="text-red-500 text-sm mt-1">{{ $message }}</span>
                        @enderror
                    ...
                    <div class="col-span-2">
                        ...
                        <select id="status" name="status"
                            ...
                            <option value="Ativo" {{ old('status', $servidor->status) == 'Ativo' ? 'selected' : '' }}>
                                Ativo</option>
                            <option value="Inativo" {{ old('status', $servidor->status) == 'Inativo' ? 'selected' : '' }}>
                                Inativo</option>
                        </select>
                    </div>
                </div>
